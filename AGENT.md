...
2. `engine` 和 `book` 允许直接复制 GUI 后端代码过来，再做适配。
3. `xiangqi` 核心围绕 `u8[90]` 重写，关键访问函数尽量 `#[inline]`。
4. `ui` 只负责渲染和命中，不直接承载棋规与引擎逻辑。
5. `app` / `game` 负责交互编排与状态汇总。
6. 需要跨模块的业务访问入口，优先收口到薄 `service` 层，不要把逻辑散进 `app`。
7. 内部数据交互统一使用 Rust 内存结构，不做前后端式 JSON 数据流。
